...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^caf_") ; then
       CAF_BUILD=$(echo -n $1 | sed -e 's/^caf_//g')
    else
       CAF_BUILD=
    fi
    export CAF_BUILD
...
{
    target=$1
    CAF_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/caf/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the caf model name
            lunch caf_$target-userdebug
        fi
    fi
    return $?
}
